...
// ── Small reusable inline-editable field for the detail panel ──
function EditableField({ label, value, onChange, mono = false, multiline = false }) {
  ...
  return (
    <div onClick={() => { setDraft(value); setEditing(true); }}
      ...
      <span style={{ fontSize: 10, color: 'var(--color-text-tertiary)', opacity: 0, marginTop: 3 }} className="edit-hint">✎</span>
    </div>
  );
}

// ── Editable select for detail panel ──
...
// ── Custom dropdown with add/delete ──
...
export default function App() {
  const [projects, setProjects] = useState(() => { try { const s = localStorage.getItem('errlog_projects'); return s ? JSON.parse(s) : []; } catch { return []; } });
  const [errors, setErrors] = useState(() => { try { const s = localStorage.getItem('errlog_errors'); return s ? JSON.parse(s) : []; } catch { return []; } });
  const [activeProj, setActiveProj] = useState(() => { try { const s = localStorage.getItem('errlog_activeProj'); return s ? JSON.parse(s) : null; } catch { return null; } });
  ...
  const [nextId, setNextId] = useState(() => { try { const s = localStorage.getItem('errlog_nextId'); return s ? JSON.parse(s) : 1; } catch { return 1; } });
  ...
  // Resolution expand state (error id -> bool)
  const [resExpanded, setResExpanded] = useState({});

  // Custom tool/lang lists
  ...
  useEffect(() => { localStorage.setItem('errlog_projects', JSON.stringify(projects)); }, [projects]);
  useEffect(() => { localStorage.setItem('errlog_errors', JSON.stringify(errors)); }, [errors]);
  useEffect(() => { localStorage.setItem('errlog_nextId', JSON.stringify(nextId)); }, [nextId]);
  ...
  useEffect(() => { localStorage.setItem('errlog_activeProj', JSON.stringify(activeProj)); }, [activeProj]);
  useEffect(() => { localStorage.setItem('errlog_sidebarCollapsed', JSON.stringify(sidebarCollapsed)); }, [sidebarCollapsed]);

  // Ensure activeProj points to a valid project (e.g. after import or clear)
  useEffect(() => {
    if (projects.length > 0 && !projects.find(p => p.id === activeProj)) {
      setActiveProj(projects[0].id);
    }
    if (projects.length === 0) setActiveProj(null);
  }, [projects]);

  const fileInputRef = useRef(null);

  function exportData() {
    const data = { projects, errors, nextId, toolList, langList };
  ...
  function importData(e) {
    ...
    reader.onload = ev => {
      try {
        ...
        if (Array.isArray(data.toolList)) setToolList(data.toolList);
        if (Array.isArray(data.langList)) setLangList(data.langList);
        if (Array.isArray(data.projects) && data.projects.length > 0) setActiveProj(data.projects[0].id);
        setDetailId(null);
  ...
  function submitError() {
    ...
    const entry = {
      ...
      resolutionTitle: '', resolution: '', resolved: false, tags: editP.tags || [],
  ...
  function createProject() {
    ...
    const p = { id: nextId, name: newProjName.trim(), color: newProjColor, phase: newProjPhase };
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'var(--font-sans, system-ui)', background: 'var(--color-background-tertiary)', color: 'var(--color-text-primary)', overflow: 'hidden' }}>
      ...
      <div style={{ width: sidebarCollapsed ? 44 : 220, minWidth: sidebarCollapsed ? 44 : 220, background: 'var(--color-background-primary)', borderRight: '0.5px solid var(--color-border-tertiary)', display: 'flex', flexDirection: 'column', overflowY: 'auto', transition: 'width 0.2s, min-width 0.2s', overflow: 'hidden' }}>
        ...
        {/* Collapsed: just project color dots */}
        {sidebarCollapsed ? (
          <div style={{ padding: '8px 0', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
            ...
            <div style={{ marginTop: 'auto', paddingTop: 12, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
              ...
              <button onClick={() => fileInputRef.current?.click()} title="Import backup" style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, color: 'var(--color-text-tertiary)', lineHeight: 1 }}>↓</button>
          ...
          <>
            ...
            <div style={{ marginTop: 'auto', padding: '14px 14px 18px', borderTop: '0.5px solid var(--color-border-tertiary)' }}>
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 12 }}>
                ...
                <button onClick={() => fileInputRef.current?.click()} title="Import backup JSON"
                  style={{ flex: 1, padding: '5px 0', borderRadius: 6, border: '0.5px solid var(--color-border-secondary)', background: 'transparent', color: 'var(--color-text-secondary)', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>
                  ↓ Import
                </button>
